...
const { monitorInfo: { user, token, domain } = {}, companyInfo } = userStore;
...
const handleView = row => {
  ...
  const src = `${
    domain ? 'https:' + domain : companyInfo.monitor_ip + '/'
  }archive/media${cameraId.replace('hosts', '')}`;
  const query = domain ? `?format=MJPEG&authToken=${token}` : '';
  const srcArr = [`${src}/${time + query}`, `${src.replace(/:0$/, ':1')}/${time + query}`];
